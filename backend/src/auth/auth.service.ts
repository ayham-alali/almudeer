...
export class AuthService {
  // Using an in-memory map for cache (could be swapped with Redis or CacheManager in production)
  ...
  async register(dto: RegisterDto) {
    ...
    // Mock Email sending
    console.log(`[MOCK EMAIL] To: ${user.email} -> Your Verification OTP is: ${otp}`);
  ...
  async verifyEmailOtp(dto: VerifyOtpDto) {
    ...
    this.otpCache.delete(dto.email); // Clear OTP after success
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    const user = await this.usersService.findByEmailOrUsername(dto.email);
    if (!user) {
      throw new NotFoundException('User not found'); // In production, we might just return success to prevent email discovery
    }
